...
def add_family_name_to_cleaned(file_path, df_with_family):
    ...
    If FAMILY_NAME already exists, it overwrites the column with new values.
    ...
    # If FAMILY_NAME exists, overwrite it with the new values
    if 'FAMILY_NAME' in df_cleaned.columns:
        print("FAMILY_NAME already exists. Overwriting the column.")
        df_cleaned = df_cleaned.drop(columns=['FAMILY_NAME'])
    
    df_selected = df_with_family[['TREE_ID', 'FAMILY_NAME']]

    # Merge the family names on TREE_ID, overwrite if FAMILY_NAME exists
    df_cleaned = df_cleaned.merge(df_selected, on="TREE_ID", how="right")

    # Save the updated dataframe to the CSV
    df_cleaned.to_csv(file_path, index=False)
    
    print("FAMILY_NAME added/overwritten in CSV.")